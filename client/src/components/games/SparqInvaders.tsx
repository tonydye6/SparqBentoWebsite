...
export function SparqInvaders() {
  ...
  useEffect(() => {
    ...
    // Load images
    const playerImagePromise = loadImage('/game_hero.png');
    const enemyImagesPromises = Array.from({ length: 8 }, (_, i) => loadImage(`/invader_${i + 1}.png`));
    const backgroundImagePromise = loadImage('/field_background.png');

    Promise.all([playerImagePromise, ...enemyImagesPromises, backgroundImagePromise]).then(([playerImage, ...enemyImages]) => {
        const enemyImagesArray = enemyImages.filter(img => img !== undefined) as HTMLImageElement[];
        ...
        function drawGame() {
          ...
          ctx.drawImage(enemyImages[0], 0, 0, canvas.width, canvas.height);
